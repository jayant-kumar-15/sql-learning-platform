...
(function () {
    ...
    async function loadAnalytics() {
         ...
         * WEEKLY SAFETY NET
         * -----------------
         * The backend should return one row per ISO week. If an older
         * deployment/cache sends daily rows while Weekly is selected,
         * aggregate those rows here before drawing the chart.
         *
         * This keeps the Weekly UI correct without changing Daily,
         * Monthly, Yearly, Feedback or Data Management behaviour.
         */
        if (
            requestedRange === "12w" &&
            series.length &&
            ...
            series.forEach(function (row) {
                const rawDate = String(row.label || "");
                ...
                const weekLabel =
                    isoYear + "-W" + String(weekNumber).padStart(2, "0");
                ...
                const target = weeklyMap.get(weekLabel);
                target.page_visits += Number(row.page_visits || 0);
                target.unique_sessions += Number(row.unique_sessions || 0);
            });

            series = Array.from(weeklyMap.values());

            console.info(
                "Weekly analytics normalized on the dashboard.",
                series
            );
        }
