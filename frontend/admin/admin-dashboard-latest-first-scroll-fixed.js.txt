 ...
 * 5. Opens the chart at the newest/rightmost data on first load;
 *    earlier data is revealed by scrolling LEFT.
 * 6. Tooltip shows date/week, page views and unique sessions.
 * 7. Window resize redraws the current chart without making a
...
(function () {
    ...
    function drawChart(rows, resetScroll) {
        ...
        /*
         * Show the newest dates immediately.
         *
         * The chart remains chronological from left to right.
         * The viewport simply opens at the newest/rightmost side.
         * Users can scroll LEFT to reveal earlier days/weeks.
         */
        if (resetScroll) {
            /*
             * Open at the newest data.
             *
             * The chart canvas is wider than the visible wrapper.
             * Browsers can occasionally report the old scrollWidth
             * during the first animation frame, especially on mobile.
             * Use two frames plus a short timeout so the final layout
             * has definitely been applied before moving to the right.
             *
             * Result:
             *   Initial view  -> latest/today on the right
             *   Scroll LEFT  -> yesterday, previous days and older weeks
             */
            const scrollToLatest = function () {
                ...
                wrapper.scrollLeft = latestPosition;

                /*
                 * scrollTo() reinforces the position on mobile browsers
                 * where direct scrollLeft assignment can be delayed
                 * during canvas/layout updates.
                 */
    ...
    async function loadAnalytics() {
         ...
         * changes. This is especially important on mobile browsers
         * and after deploying a new analytics version.
         */
         ...
         *
         * Weekly must contain weekly labels. If an old backend build
         * accidentally returns daily labels for 12w, do not silently
         * pretend those rows are weekly data.
         */
        ...
        if (
            ...
            series.some(function (row) {
                return !String(row.label || "").includes("-W");
            })
        ) {
            console.warn(
                "Weekly analytics returned non-weekly labels. " +
                "Expected labels such as 2026-W36."
            );
        }
