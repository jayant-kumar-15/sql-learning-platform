...
(function () {
    ...
    async function loadFeedback() {
        ...
        // Always replace the local copy with the latest backend data.
        // This is important because public users can submit feedback while
        // the admin dashboard is already open in another tab/device.
        state.feedback = Array.isArray(data.rows)
            ? data.rows
    ...
    function bindEvents() {
        document.querySelectorAll(".nav-item").forEach(function (button) {
            button.addEventListener("click", async function () {
                ...
                // Fetch fresh messages every time the Feedback section opens.
                // This prevents newly submitted public messages from being
                // hidden until the entire dashboard page is reloaded.
                if (section === "feedback") {
                    try {
                        await loadFeedback();
                    } catch (error) {
                        console.error("Could not refresh feedback:", error);
                        const list = $("#feedbackList");
                        list.innerHTML =
                            "<p class='empty-state'>Unable to load messages right now.</p>";
                    }
                }
            });
        });
        ...
        document.querySelectorAll(".filter-button").forEach(function (button) {
            button.addEventListener("click", function () {
                ...
                // The filter only changes presentation; all categories remain
                // available from the latest backend response.
                renderFeedback();
            });
        });
        ...
        // Keep the Feedback section current without requiring manual reloads.
        // This is intentionally lightweight: one request every 30 seconds.
