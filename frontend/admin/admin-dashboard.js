...
(function () {
     ...
     * ------------------------------------------------------------
     * The backend already protects this endpoint with admin auth.
     * This UI lets the administrator move each message between:
     *     new -> read -> resolved
     * without leaving the current filter or reloading the page.
     * ============================================================ */
    async function updateFeedbackStatus(feedbackId, status, selectElement) {
        const previousStatus = selectElement.dataset.previousStatus || "new";

        selectElement.disabled = true;

        try {
            await api("/api/admin/feedback/" + encodeURIComponent(feedbackId), {
                method: "PATCH",
                headers: {
            ...
            const item = state.feedback.find(function (feedback) {
            ...
            /* Keep the overview counter synchronized immediately. */
            const unread = state.feedback.filter(function (feedback) {
            ...
            alert("Unable to update the feedback status. Please try again.");
        } finally {
    ...
    function renderFeedback() {
        ...
        items.forEach(function (item) {
            ...
            card.innerHTML =
                ...
                "<label for='feedback-status-" + escapeHtml(item.id) + "'>Status</label>" +
                "<select id='feedback-status-" + escapeHtml(item.id) + "' aria-label='Feedback status'>" +
            ...
            const currentStatus = ["new", "read", "resolved"].includes(item.status)
                ? item.status
                : "new";
            ...
            statusSelect.addEventListener("change", function () {
                updateFeedbackStatus(item.id, statusSelect.value, statusSelect);
            });
