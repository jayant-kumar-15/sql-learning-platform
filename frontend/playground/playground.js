...
/* Mobile main-navigation control is intentionally separate from
 * the Database Explorer control above. */
...
function renderRelationshipDiagram(database) {
     ...
     * ------------------------------------------------------------
     * The previous layout used one fixed estimated card height for
     * every table. That caused taller tables such as `loans` to
     * extend into the RELATIONSHIPS section and caused cards such
     * as `payments` / `transactions` to overlap the row above.
     *
     * Keep the existing 4-column visual layout, but calculate each
     * row's height from the tallest table in that row. This gives
     * every card enough vertical space without changing the
     * relationship diagram's visual style.
     */
    ...
    // Keep these values synchronized with the CSS card structure:
    // header ≈ 36px + body padding ≈ 10px + 26px per column.
    ...
    const getCardHeight = (table) => {
        ...
        return Math.max(
            cardMinHeight,
            cardHeaderHeight + cardBodyPadding + (columnCount * cardColumnHeight)
        );
    };

    const rowCount = Math.max(1, Math.ceil(tables.length / columnsPerRow));
    const rowHeights = Array.from({ length: rowCount }, () => cardMinHeight);

    // Determine the required height of every row before positioning cards.
    tables.forEach((table, index) => {
        const row = Math.floor(index / columnsPerRow);
        rowHeights[row] = Math.max(rowHeights[row], getCardHeight(table));
    });
    ...
    const totalRowGaps = Math.max(0, rowCount - 1) * cardGapY;

    // Extra bottom space keeps the last card visually separated from
    // the scrollable RELATIONSHIPS list below the canvas.
    const canvasHeight = Math.max(
        540,
        totalRowsHeight + totalRowGaps + 70
    );
    ...
    tables.forEach((table, index) => {
        ...
        // Sum the actual heights of previous rows instead of using
        // one fixed card-height estimate.
        const previousRowsHeight = rowHeights
            .slice(0, row)
            .reduce((sum, height) => sum + height, 0);

        const y = 35 + previousRowsHeight + (row * cardGapY);
...
function initializeEvents() {
    ...
    /* Mobile main navigation is independent from Database Explorer. */
 ...
 *
 * IMPORTANT:
 * - This controls ONLY the platform navbar.
 * - It does NOT open the Database Explorer.
 * - The Database Explorer keeps its existing dedicated button.
 * ============================================================ */

function toggleMobileNavigation() {
    if (!platformNav || !mobileNavButton) return;

    const isOpen = platformNav.classList.toggle("mobile-nav-open");
...
function closeMobileNavigation() {
    ...
    platformNav.classList.remove("mobile-nav-open");
