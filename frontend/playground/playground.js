...
function updateFooterYear() {
    const yearElement = document.getElementById("playground-footer-year");
    if (yearElement) {
...
function updateActiveDatabaseUI() {
    ...
    /* Mobile UI can show the active pre-built database even while the
       full Explorer is closed. This is presentation-only state. */
    document.body.dataset.activeDatabase = name;
    const banner = document.querySelector(".playground-info-banner");
    if (banner) banner.dataset.activeDb = name;

    /* Keep the selected pre-built database/table visible on mobile. */
    const mobileDatabase = document.getElementById("mobile-active-database");
    const mobileTable = document.getElementById("mobile-active-table");

    if (mobileDatabase) {
        mobileDatabase.textContent = name;
    }

    if (mobileTable) {
        mobileTable.textContent = activeTable?.table || "Select from Explorer";
    }
