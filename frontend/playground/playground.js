 ...
 * - Restores Run Query below the editor on mobile.
 * - Positions Database Explorer immediately during startup.
 * - Re-checks both controls after load/pageshow/resize.
 * - Leaves SQL execution, databases, query tabs, results, footer and
...
const mobileRunQueryActions = document.getElementById("mobile-run-query-actions");
...
async function initializePlayground() {
    try {
        ...
        /*
         * Position mobile controls immediately, BEFORE asynchronous database
         * loading. This prevents hard-refresh timing from leaving the
         * Database Explorer in the header or leaving Run Query hidden.
         */
        positionMobileExplorerButton();
        positionMobileRunQueryButton();
         ...
         */
        /*
         * Final synchronization after the async SQLite load completes.
         * This does not alter any database/query behavior.
         */
...
function renderRelationshipDiagram(database) {
    ...
    /*
     * RELATIONSHIP CARD LAYOUT — OVERLAP FIX ONLY
     * ------------------------------------------------------------
     * Keep the existing 4-column layout and visual sizing.
     * Calculate each row from the tallest table in that row so a
     * taller card cannot overlap the cards in the next row.
     */
    ...
    const cardBodyPadding = 10;
    const cardColumnHeight = 26;
    ...
    const getCardHeight = (table) => {
        ...
        return Math.max(
            ...
            cardHeaderHeight +
                cardBodyPadding +
                (columnCount * cardColumnHeight)
        );
    };

    const rowCount = Math.max(
        1,
        Math.ceil(tables.length / columnsPerRow)
    );
    ...
    const totalRowGaps =
        Math.max(0, rowCount - 1) * cardGapY;
...
function positionMobileExplorerButton() {
    ...
    if (isMobile) {
        /*
         * Keep Explorer as a direct child of the query-tabs container,
         * immediately to the left of Query 1. Do NOT place it inside the
         * horizontally scrollable #query-tabs element.
         */
        const queryTabs = queryTabsContainer.querySelector("#query-tabs");

        if (queryTabs && explorerButton.parentElement !== queryTabsContainer) {
            queryTabsContainer.insertBefore(explorerButton, queryTabs);
        }

        return;
    }

    /* Restore the original desktop location. */
    if (
...
/* ============================================================
 * MOBILE RUN QUERY BUTTON PLACEMENT
 * ============================================================
 * One existing Run Query button is used on both layouts.
 *
 * Desktop (> 760px):
 *   Keep it in .workspace-actions.
 *
 * Mobile (<= 760px):
 *   Move it below the SQL editor into #mobile-run-query-actions.
 *
 * The DOM node is moved, not recreated, so its existing click listener
 * and SQL execution logic remain unchanged.
 * ============================================================ */
function positionMobileRunQueryButton() {
    if (!runQueryButton || !mobileRunQueryActions) return;
    ...
    const target = isMobile
        ? mobileRunQueryActions
        : workspaceActions;

    if (runQueryButton.parentElement !== target) {
        target.appendChild(runQueryButton);
    }
}

/* ============================================================
 * RESPONSIVE CONTROL SYNCHRONIZATION
 * ============================================================
 * Re-check both controls after load, page restore, and viewport changes.
 * This specifically protects Chrome Android hard-refresh/page-cache timing.
 * ============================================================ */
...
window.addEventListener("resize", synchronizeMobileControls, { passive: true });
window.addEventListener("load", synchronizeMobileControls, { passive: true });
window.addEventListener("pageshow", synchronizeMobileControls, { passive: true });
