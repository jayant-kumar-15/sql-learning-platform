...
/* ============================================================
 * DATABASE INITIALIZATION & INTROSPECTION
 * ============================================================ */
...
function readSchemaSql(db, tableName) {
    ...
    return rows[0]?.sql || `CREATE TABLE ${quoteIdentifier(tableName)} (...);`;
}
...
function readIndexes(db, tableName) {
    return db.exec({
        sql: `
            ...
            FROM sqlite_master
            WHERE type = 'index'
              AND tbl_name = ${quoteString(tableName)}
            ORDER BY name
        `,
        rowMode: "object",
        returnValue: "resultRows"
...
function renderDatabaseTree() {
    ...
    databases.forEach(database => {
        ...
        const shouldShowTables = expandedDatabases.has(database.name) || (query && matchingTables.length > 0);
        ...
        header.addEventListener("click", async event => {
            if (event.target.closest(".database-arrow")) return;
...
/* ============================================================
 * SQL STATEMENT PARSING & VALIDATION
 * ============================================================ */
...
async function executeCurrentQuery(options = {}) {
    ...
    try {
        ...
        for (const statement of target.statements) {
            const useMatch = statement.match(/^USE\s+["'`]?([A-Za-z0-9_-]+)["'`]?\s*$/i);
...
function renderSchema(table) {
    ...
    detailsContainer.innerHTML = `
        ...
        <section class="schema-definition-card" style="margin-bottom: 12px;">
            <div style="margin-bottom: 6px; color: #93c5fd; font-size: 10px; font-weight: 700;">CREATE TABLE</div>
            <pre style="margin: 0; padding: 12px; border: 1px solid #263247; border-radius: 6px; background: #07101f; color: #cbd5e1; font-family: monospace; font-size: 11px; white-space: pre-wrap; overflow: auto;">${escapeHTML(table.schemaSql || "No CREATE TABLE statement available.")}</pre>
        </section>

        <section class="schema-extra-section">
            <div style="margin-bottom: 6px; color: #93c5fd; font-size: 10px; font-weight: 700;">FOREIGN KEYS</div>
            ${foreignKeys.length ? `
                <div style="border: 1px solid #263247; border-radius: 6px; background: #0b1120; overflow: hidden;">
                    ${foreignKeys.map(fk => `
                        <div style="padding: 7px 10px; border-bottom: 1px solid #1e293b; color: #cbd5e1; font-size: 11px; display: flex; gap: 8px;">
                            <span>${escapeHTML(fk.from)}</span>
                            <span style="color: #60a5fa;">→</span>
                            <strong style="color: #93c5fd;">${escapeHTML(fk.table)}.${escapeHTML(fk.to)}</strong>
                        </div>
                    `).join("")}
                </div>
            ` : '<div style="padding: 10px; border: 1px dashed #334155; border-radius: 5px; color: #64748b; font-size: 10px;">No foreign keys defined for this table.</div>'}
        </section>
    `;
...
/* ============================================================
 * RELATIONSHIPS / DATA MODEL VIEWER
 * ============================================================ */
...
function renderRelationshipDiagram(database) {
    ...
    drawRelationshipLines(database, positions);
}
            ...
            <div class="relationship-table-body">
                ${(table.columns || []).map(column => `
                    <div class="relationship-column">
                        ...
                        <span style="color: #64748b; font-family: monospace; font-size: 9px;">${escapeHTML(column.type || "")}</span>
                    </div>
                `).join("")}
            </div>
        </article>
...
function openMobileSidebar() {
    ...
    mobileSidebarOverlay?.classList.add("active", "visible");
}

function closeMobileSidebar() {
    document.getElementById("database-sidebar")?.classList.remove("mobile-open");
    mobileSidebarOverlay?.classList.remove("active", "visible");
}
