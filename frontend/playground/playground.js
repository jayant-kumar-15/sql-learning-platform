...
/* ============================================================
 * RUN QUERY BUTTON RESPONSIVE PLACEMENT
 * ------------------------------------------------------------
 * There is only ONE Run Query button and therefore only ONE
 * JavaScript event handler.
 *
 * Desktop (> 760px)
 * -----------------
 * Keep the button in the original SQL WORKSHEET toolbar beside
 * Describe / Schema / Relationships.
 *
 * Mobile (<= 760px)
 * ------------------
 * Move that same button into the Challenge-style editor action
 * area below the SQL editor. This preserves the mobile UI that
 * was already working while restoring the desktop layout.
 *
 * Moving the existing DOM node does NOT recreate the button and
 * does NOT remove its click listener.
 * ============================================================ */
function placeRunQueryButton() {
    if (!runQueryButton) return;

    const workspaceActions = document.querySelector(".workspace-actions");
    if (!workspaceActions || !mobileRunQueryActions) return;

    const isMobile = window.matchMedia("(max-width: 760px)").matches;
    const target = isMobile ? mobileRunQueryActions : workspaceActions;
...
async function initializePlayground() {
    try {
        ...
        const footerYear = document.getElementById("playground-footer-year");
...
function maximizeResults() {
    ...
    if (resultsSection.classList.contains("results-maximized")) {
        /*
         * Desktop CSS owns the maximized boundary (inside the
         * Playground workspace). Mobile CSS owns its lower-half
         * behavior. Avoid forcing a 100% inline height here because
         * that can override the responsive boundary rules.
         */
...
function initializeEvents() {
    ...
    closeRelationships?.addEventListener("click", event => {
        event.preventDefault();
     ...
     * ------------------------------------------------------------
     * The hamburger is intentionally separate from the Database
     * Explorer button. It only toggles the existing platform navbar
     * on small screens and does not touch the SQL Playground state.
     * ------------------------------------------------------------
     */
    ...
    /* Keep the same Run Query button in the correct desktop/mobile destination. */
    window.addEventListener("resize", placeRunQueryButton);
    placeRunQueryButton();

    if (!closeRelationships || !closeDetails) {
        console.warn("Playground modal close controls are missing from the DOM.");
    }
...
function openMobileSidebar() {
    ...
    mobileSidebarButton?.setAttribute("aria-expanded", "true");
}
...
/*
 * Toggle the mobile platform navigation.
 *
 * This is deliberately independent from Database Explorer:
 * - hamburger -> platform navigation
 * - database button -> Database Explorer
 */
function toggleMobileNavigation() {
    const header = document.querySelector("body > .platform-header");
    if (!header) return;
