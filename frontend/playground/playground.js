...
async function initializePlayground() {
    try {
        ...
        /*
         * Keep every database folded on initial load.
         * The user explicitly controls expansion with the database arrow.
         * Selecting/activating a database must not automatically unfold it.
         */
...
async function setActiveDatabase(name, options = {}) {
    ...
    /*
     * IMPORTANT: changing the active database must not change its
     * expanded/collapsed state. The explorer arrow owns that state.
     */
    updateActiveDatabaseUI();
...
function renderDatabaseTree() {
    ...
    databases.forEach(database => {
        ...
        item.setAttribute(
            "aria-expanded",
            String(expandedDatabases.has(database.name))
        );

        /*
         * The database row is deliberately NOT a button. The small arrow
         * is now a real, dedicated toggle button so the fold/unfold control
         * remains visible and predictable after every refresh.
         */
        ...
        toggleButton.className = "database-toggle-button";
        toggleButton.setAttribute(
            "aria-label",
            `${expandedDatabases.has(database.name) ? "Collapse" : "Expand"} ${database.name}`
        );
        toggleButton.title = expandedDatabases.has(database.name)
            ? `Collapse ${database.name}`
            : `Expand ${database.name}`;
        toggleButton.innerHTML = `<span class="database-arrow">${expandedDatabases.has(database.name) ? "▼" : "▶"}</span>`;
        ...
        name.title = database.name;

        const activeBadge = document.createElement("span");
        activeBadge.className = "database-active-badge";
        activeBadge.textContent = "ACTIVE";
        activeBadge.hidden = database.name !== activeDatabase;

        header.append(toggleButton, icon, name, activeBadge);

        const tableList = document.createElement("div");
        tableList.className = "table-list";
        tableList.hidden = !(expandedDatabases.has(database.name) || (query && matchingTables.length > 0));
        tableList.setAttribute("aria-hidden", String(tableList.hidden));
        ...
        database.tables.forEach(table => {
            ...
            if (
                activeTable?.database === database.name &&
            ...
            tableElement.addEventListener("click", async event => {
                ...
                try {
                    ...
                    /* Selecting a table must never change fold/unfold state. */
                    renderDatabaseTree();
                    showStatus(
                        `Selected table: ${database.name}.${table.name}`,
                        "info"
                    );
                } catch (error) {
        ...
        /* Clicking the database name activates it, but does NOT unfold it. */
        name.addEventListener("click", async event => {
            event.stopPropagation();

            try {
                await setActiveDatabase(database.name, { silent: true });
                showStatus(`Active database: ${database.name}`, "success");
            } catch (error) {
                showStatus(error.message, "error");
            }
        });

        item.append(header, tableList);
