 ...
 * mobile. The existing element is reused; no duplicate button is
 * created and no SQL/Explorer functionality is changed.
 */
function positionMobileExplorerButton() {
    ...
    const queryTabsElement = document.getElementById("query-tabs");

    if (!explorerButton || !queryTabsContainer) return;

    /*
     * IMPORTANT — MOBILE DETECTION
     * ------------------------------------------------------------
     * Chrome on Android can report a desktop-sized CSS viewport when
     * "Desktop site" is enabled. In that situation max-width: 760px
     * is FALSE even though the physical device is a phone.
     *
     * playground.html adds `force-mobile-shell` for that case. Treat
     * it exactly like normal mobile so the Explorer button is moved
     * into the Query 1 bar instead of remaining fixed beside the
     * platform header.
     * ------------------------------------------------------------
     */
    const isMobileLayout =
        window.matchMedia("(max-width: 760px)").matches ||
        document.documentElement.classList.contains("force-mobile-shell");

    if (isMobileLayout) {
        /* Move the EXISTING button into the query-tab container. */
        if (explorerButton.parentElement !== queryTabsContainer) {
            queryTabsContainer.insertBefore(
                explorerButton,
                queryTabsContainer.firstElementChild
            );
        }

        /*
         * For a real mobile viewport, playground.css provides these
         * styles. For Chrome Desktop-site mode, the max-width media
         * query may not run, so apply the same positioning directly.
         * `position: absolute` is intentional: it is anchored to the
         * Query 1 bar and therefore scrolls away with it, just like
         * Query 1. It is NOT viewport-fixed.
         */
        if (document.documentElement.classList.contains("force-mobile-shell")) {
            queryTabsContainer.style.position = "relative";
            queryTabsContainer.style.width = "100%";
            queryTabsContainer.style.minHeight = "40px";
            queryTabsContainer.style.height = "40px";
            queryTabsContainer.style.display = "flex";
            queryTabsContainer.style.alignItems = "stretch";
            queryTabsContainer.style.overflow = "visible";

            explorerButton.style.position = "absolute";
            explorerButton.style.top = "2px";
            explorerButton.style.left = "12px";
            explorerButton.style.width = "44px";
            explorerButton.style.height = "36px";
            explorerButton.style.margin = "0";
            explorerButton.style.padding = "0";
            explorerButton.style.display = "inline-flex";
            explorerButton.style.alignItems = "center";
            explorerButton.style.justifyContent = "center";
            explorerButton.style.zIndex = "1004";

            if (queryTabsElement) {
                queryTabsElement.style.width = "100%";
                queryTabsElement.style.paddingLeft = "68px";
                queryTabsElement.style.boxSizing = "border-box";
            }
        }
    ...
    /*
     * Remove only the inline styles that were added for
     * force-mobile-shell. Desktop CSS is then free to control the
     * button exactly as before.
     */
    if (!document.documentElement.classList.contains("force-mobile-shell")) {
        queryTabsContainer.style.position = "";
        queryTabsContainer.style.width = "";
        queryTabsContainer.style.minHeight = "";
        queryTabsContainer.style.height = "";
        queryTabsContainer.style.display = "";
        queryTabsContainer.style.alignItems = "";
        queryTabsContainer.style.overflow = "";

        explorerButton.style.position = "";
        explorerButton.style.top = "";
        explorerButton.style.left = "";
        explorerButton.style.width = "";
        explorerButton.style.height = "";
        explorerButton.style.margin = "";
        explorerButton.style.padding = "";
        explorerButton.style.display = "";
        explorerButton.style.alignItems = "";
        explorerButton.style.justifyContent = "";
        explorerButton.style.zIndex = "";

        if (queryTabsElement) {
            queryTabsElement.style.width = "";
            queryTabsElement.style.paddingLeft = "";
            queryTabsElement.style.boxSizing = "";
        }
    }
}
