...
function renderDatabaseTree() {
    ...
    databases.forEach(database => {
        ...
        item.setAttribute("aria-expanded", String(expandedDatabases.has(database.name)));

        const header = document.createElement("button");
        header.type = "button";
        header.className = "database-header";
        header.innerHTML = `
            <span class="database-arrow">${expandedDatabases.has(database.name) ? "▼" : "▶"}</span>
        ...
        tableList.style.display = expandedDatabases.has(database.name) || (query && matchingTables.length > 0) ? "block" : "none";
        ...
        database.tables.forEach(table => {
            ...
            if (activeTable?.database === database.name && activeTable?.table === table.name) {
...
function renderRelationshipDiagram(database) {
    ...
    /*
     * RELATIONSHIP CARD LAYOUT — FIX FOR TABLE OVERLAP
     * ------------------------------------------------------------
     * The previous layout used one fixed height for every table.
     * Tables with more columns could therefore extend into the next
     * row and visually overlap cards such as customers/transactions.
     *
     * Keep the existing visual design and 4-column arrangement.
     * Only calculate each row from the tallest table in that row
     * and add a safe vertical gap between rows.
     */
    ...
    // Determine the actual required height of each row.
    ...
    tables.forEach((table, index) => {
        ...
        // Start this row below the actual height of all
        // previous rows, preventing table-card overlap.
