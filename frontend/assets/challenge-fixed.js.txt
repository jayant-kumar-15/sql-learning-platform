...
/* ============================================================
 * SHARE CHALLENGE DOM ELEMENTS
 * ------------------------------------------------------------
 * These references are intentionally declared with the other DOM
 * elements before any share event listeners are registered.
 * This prevents a JavaScript temporal-dead-zone error from stopping
 * the rest of the Challenge page initialization.
 * ============================================================
 */

const shareChallengeButton =
    document.getElementById("share-challenge-btn");

const shareChallengeOverlay =
    document.getElementById("share-challenge-overlay");

const shareChallengePopup =
    document.getElementById("share-challenge-popup");

const closeShareChallengeButton =
    document.getElementById("close-share-challenge-btn");

const shareChallengeSummary =
    document.getElementById("share-challenge-summary");

const shareChallengePreview =
    document.getElementById("share-challenge-preview");

const shareLinkedInButton =
    document.getElementById("share-linkedin-btn");
...
const shareCopyLinkButton =
    document.getElementById("share-copy-link-btn");

const shareNativeButton =
    document.getElementById("share-native-btn");

const shareCopyStatus =
    document.getElementById("share-copy-status");
 ...
 * SHARE URL + MESSAGE
 * ------------------------------------------------------------
 * No backend call is required. Sharing is generated entirely in
 * the browser, so this feature does not affect the existing SQL
 * engine, progress storage, authentication, or analytics.
 * ============================================================
 */
function getChallengeShareUrl(question) {
...
function getChallengeShareText(question) {
    ...
    const questionTextValue =
        question && question.question
            ...
            : "Can you solve this SQL challenge?";

    const shareUrl =
        getChallengeShareUrl(question);

    /*
     * One canonical share message is used everywhere.
     * This keeps WhatsApp, X, native sharing, and Copy Message & Link
     * consistent with the message shown in the Share Challenge popup.
     */
    return (
        "I dare you to solve this SQL challenge! 🔥\n" +
        "Question " +
        questionNumber +
        " • " +
        difficulty +
...
/* ============================================================
 * SHARE CHALLENGE — MODAL SAFETY + RESPONSIVE VISUAL FIX
 * ------------------------------------------------------------
 * This small block is intentionally isolated to the Share Challenge
 * feature. It guarantees that the share dialog behaves as a true
 * viewport modal even if an older/cached challenge.css is still being
 * served by GitHub Pages.
 *
 * No SQL execution, challenge progression, question rendering,
 * navigation, schema viewer, or other Challenge behavior is changed.
 * ============================================================ */
(function ensureShareChallengeStyles() {

    if (document.getElementById("share-challenge-js-fix-style")) {
        return;
    }

    const style = document.createElement("style");
    style.id = "share-challenge-js-fix-style";

    style.textContent = `
        /* Share button on the question card */
        .share-challenge-btn {
        ...
        /* True centered popup — never part of normal page flow */
...
function closeShareChallengePopup() {

    document.body.style.overflow = "";

    if (shareChallengePopup) {
        shareChallengePopup.style.display = "none";
    }

    if (shareChallengeOverlay) {
        shareChallengeOverlay.style.display = "none";
        shareChallengeOverlay.setAttribute(
            "aria-hidden",
            "true"
        );
    }
...
function openShareChallengePopup() {

    if (!currentQuestion || !shareChallengePopup) {
        return;
    }

    if (shareChallengeSummary) {
        shareChallengeSummary.textContent =
            "Question " +
            currentQuestion.id +
            " • " +
            (currentQuestion.difficulty || "SQL Challenge") +
            " • " +
            (currentQuestion.database || "");
    }

    if (shareChallengePreview) {
        shareChallengePreview.textContent =
    ...
    shareChallengePopup.style.display = "block";

    /* Prevent the page behind the modal from scrolling. */
    document.body.style.overflow = "hidden";

    if (shareChallengeOverlay) {
        shareChallengeOverlay.style.display = "block";
        shareChallengeOverlay.setAttribute(
            "aria-hidden",
            "false"
        );
    ...
    /* Put keyboard focus on Close so the modal is immediately usable. */
    if (closeShareChallengeButton) {
        closeShareChallengeButton.focus();
    }
}
...
function openShareWindow(url) {
    ...
    window.open(
        url,
        "_blank",
        "noopener,noreferrer,width=760,height=700"
    );
}
...
async function copyChallengeShareLink() {
    ...
    /*
     * The Copy Link action copies the complete ready-to-share message.
     * The message includes the exact question, difficulty, database, and
     * deep-link URL so pasting it into any platform preserves the intended
     * challenge invitation instead of exposing only a bare URL.
     */
    const shareMessage =
        getChallengeShareText(
            currentQuestion
    ...
    try {
        ...
        if (shareCopyStatus) {
            shareCopyStatus.textContent =
                "✅ Challenge message + link copied! Paste it anywhere to share.";
        }

    } catch (error) {

        console.warn(
            "⚠️ Unable to copy challenge message:",
            error
        );

        if (shareCopyStatus) {
            shareCopyStatus.textContent =
                "Copy failed. Please use one of the share buttons.";
        }
    }
}


if (shareChallengeButton) {
    shareChallengeButton.addEventListener(
        "click",
        function () {
            openShareChallengePopup();
        }
    );
}
...
if (closeShareChallengeButton) {
    closeShareChallengeButton.addEventListener(
        "click",
        function () {
            closeShareChallengePopup();
        }
    );
}
...
if (shareChallengeOverlay) {
    shareChallengeOverlay.addEventListener(
        "click",
        function () {
            closeShareChallengePopup();
        }
...
if (shareCopyLinkButton) {
    shareCopyLinkButton.addEventListener(
        "click",
        function () {
            copyChallengeShareLink();
        }
    );
}
...
if (shareLinkedInButton) {
    shareLinkedInButton.addEventListener(
        "click",
        function () {
            ...
            /*
             * LinkedIn's standard share-offsite URL does not expose a
             * parameter for arbitrary post commentary. Copy the exact same
             * canonical message first, then open LinkedIn so the user can
             * paste it into the composer while retaining the professional
             * URL preview generated by LinkedIn.
             */
            copyTextToClipboard(
                getChallengeShareText(
                    currentQuestion
                ...
                if (shareCopyStatus) {
                    shareCopyStatus.textContent =
                        "✅ Challenge message copied. Paste it into the LinkedIn composer.";
                }
                ...
                console.warn(
                    "⚠️ Unable to prepare LinkedIn share message:",
                    error
                );
...
if (shareNativeButton) {
    shareNativeButton.addEventListener(
        "click",
        async function () {
            ...
            try {
                ...
                /* User cancellation is normal and needs no error UI. */
                if (error && error.name !== "AbortError") {
                    console.warn(
                        "⚠️ Native sharing failed:",
                        error
                    );
                }
            }
 ...
 * ------------------------------------------------------------
 * A shared URL contains:
 *     ?question=<id>&difficulty=<level>
 *
 * The difficulty is included because question IDs may repeat across
 * Beginner/Intermediate/Expert datasets. The existing default behavior
 * remains Beginner #1 when no valid shared-question URL is present.
 * ============================================================
 */
function openSharedQuestionFromUrl() {
    ...
    if (!sharedQuestion) {
        ...
        console.warn(
            "⚠️ Shared challenge was not found:",
            questionId,
            requestedDifficulty
        );
            ...
            function (question) {
                return (
                    String(question.difficulty || "")
                        .trim()
                        .toLowerCase() ===
                    String(sharedQuestion.difficulty || "")
                        .trim()
                        .toLowerCase()
                );
            }
...
window.addEventListener(
    ...
    function () {
        ...
        if (
            ...
            const openedSharedQuestion =
                openSharedQuestionFromUrl();

            if (!openedSharedQuestion) {
 ...
 * ---------------------------
 * The Challenge page must NEVER fetch schema metadata from the
 * backend /api/schema endpoint.
 *
 * Challenge SQL executes against the browser SQLite database that
 * is loaded from:
 ...
 *
 * Therefore the schema popup reads directly from the SAME SQLite
 * database. This keeps the displayed schema and the executable
 * schema synchronized and removes the old backend dependency.
 * ============================================================
 */
...
function openSchemaPopup() {
    ...
    schemaOverlay.style.display = "block";
}
...
function closeSchemaPopup() {
    ...
    schemaOverlay.style.display = "none";
}


/*
 * Escape a SQLite identifier safely.
 *
 * Table names come from the trusted challenge JSON, but identifiers
 * are still escaped before being placed inside PRAGMA statements.
 */
...
/*
 * Read the actual schema from the browser SQLite database.
 *
 * This is the single source of truth for the Challenge schema UI.
 * It uses SQLite PRAGMA metadata rather than a second schema file,
 * backend endpoint, or hard-coded column list.
 */
async function getChallengeSchemaFromBrowserDatabase(
    databaseName,
    tableNames
) {
    ...
    /*
     * Make sure the exact database used by Challenges is loaded.
     * initialize() is safe to call repeatedly and switches databases
     * when the current question belongs to another database.
     */
    const db =
    ...
    if (!db) {
        throw new Error(
            "Challenge SQLite database is not available."
        );
    }
    ...
    for (const tableName of tableNames) {
        ...
        if (columnRows.length === 0) {
            throw new Error(
                `Table "${tableName}" was not found in the ${databaseName} challenge database.`
            );
        }

        /* -----------------------------------------------
         * Actual foreign-key relationships from SQLite.
         * ----------------------------------------------- */
        ...
        const columns =
            columnRows.map(function (column) {
        ...
        const foreignKeys =
            foreignKeyRows.map(function (foreignKey) {
...
/*
 * Render schema metadata returned directly by SQLite.
 */
function renderTableSchema(schemaData) {
    ...
    if (
        !schemaData ||
        !Array.isArray(schemaData.tables) ||
    ...
    schemaData.tables.forEach(function (table) {
        ...
        columns.forEach(function (column) {
            ...
            if (column.pk === 1) {
                keyText = "🔑 Primary Key";
                keyClass = "schema-primary-key";
            }

            const foreignKey =
                (table.foreignKeys || []).find(
                    function (fk) {
                        return fk.from === column.name;
                    }
                );

            if (foreignKey) {
                keyText = "🔗 Foreign Key";
                keyClass = "schema-foreign-key";
            }
...
/*
 * Load schema for the current challenge.
 *
 * REMOVED DEPENDENCY:
 *     the remote /api/schema backend endpoint
 *
 * The schema now comes from the exact browser SQLite instance used
 * to execute the Challenge query.
 */
async function loadTableSchema() {
    ...
    if (schemaContent) {
        schemaContent.innerHTML = `
            <p>
                ⏳ Reading the live Challenge database schema...
            </p>
        `;
    }
    ...
    try {
        ...
        const schemaData =
            await getChallengeSchemaFromBrowserDatabase(
                database,
                tables
            );

        renderTableSchema(schemaData);

        console.log(
            "✅ Challenge schema loaded directly from browser SQLite:",
            database,
            tables
        );
        ...
        console.error(
            "❌ Challenge schema loading failed:",
            error
        );
...
console.log(
    "✅ Challenge schema viewer now uses browser SQLite only."
);
 ...
 * ============================================================
 * The Challenges page owns its hamburger navigation in the page
 * markup. This small listener keeps the existing navigation logic
 * intact and only closes the expanded mobile menu when the user
 * starts scrolling.
 *
 * Desktop behavior and all existing Challenge functionality remain
 * unchanged.
 * ============================================================ */
window.addEventListener("scroll", () => {
    const navigation =
        document.getElementById("challengeMainNavigation");

    const menuButton =
        document.getElementById("challengeMobileMenuButton");
