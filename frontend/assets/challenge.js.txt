...
function updateActiveLevelStatus() {
    ...
    if (!activeLevelCard || !activeLevelStatus) return;

    const beginnerCompleted = getCompletedCountForDifficulty("Beginner");
    const intermediateCompleted = getCompletedCountForDifficulty("Intermediate");
    const expertCompleted = getCompletedCountForDifficulty("Expert");

    /* Hide the entire card until a meaningful milestone is earned. */
    if (expertCompleted < YOUR_LEVEL_THRESHOLDS.Expert && intermediateCompleted < YOUR_LEVEL_THRESHOLDS.Intermediate && beginnerCompleted < YOUR_LEVEL_THRESHOLDS.Beginner) {
        activeLevelCard.hidden = true;
    ...
    if (expertCompleted >= YOUR_LEVEL_THRESHOLDS.Expert) {
        const current = EXPERT_RANK_MILESTONES.reduce((latest, milestone) => expertCompleted >= milestone.completed ? milestone : latest, null);
        statusText = current ? `Expert - ${current.badge} ${current.rank}` : "Expert";
    } else if (intermediateCompleted >= YOUR_LEVEL_THRESHOLDS.Intermediate) {
        const current = INTERMEDIATE_STAR_MILESTONES.reduce((latest, milestone) => intermediateCompleted >= milestone.completed ? milestone : latest, null);
        statusText = current ? `Intermediate - ${"⭐".repeat(current.stars)}` : "Intermediate";
    } else {
        statusText = `Easy - ${beginnerCompleted}/50`;
    }

    activeLevelStatus.textContent = statusText;
    activeLevelCard.hidden = false;
}

function updateChallengeProgression() {

    updateIntermediateProgression(getCompletedCountForDifficulty("Intermediate"));
    updateExpertProgression(getCompletedCountForDifficulty("Expert"));
 ...
 * MOBILE NAVIGATION
 * ------------------------------------------------------------
 * Browser-only responsive menu. No API/backend call is involved.
 * ============================================================ */
const mobileMenuToggle = document.getElementById("challenge-mobile-menu-toggle");
const mainNav = document.getElementById("challenge-main-nav");
if (mobileMenuToggle && mainNav) {
    mobileMenuToggle.addEventListener("click", function () {
        const open = mobileMenuToggle.getAttribute("aria-expanded") === "true";
        mobileMenuToggle.setAttribute("aria-expanded", String(!open));
        mobileMenuToggle.setAttribute("aria-label", open ? "Open navigation menu" : "Close navigation menu");
        mainNav.classList.toggle("mobile-menu-open", !open);
    });
