...
/*
 * Defensive browser-side initialization:
 * keep the card hidden even if an older cached stylesheet is present.
 */
hideActiveLevelCard();

/*
 * YOUR LEVEL VISIBILITY RULE
 * --------------------------
 * Do not show the Your Level card for a new/low-progress learner.
 *
 * The card becomes visible only after one of these real milestones:
 *   1. Beginner  -> all 50 questions completed
 *   2. Intermediate -> at least 20 questions completed
 *   3. Expert -> at least 10 questions completed
 *
 * This prevents both:
 *   • the old "Easy - 0/50" flash on refresh
 *   • showing "Your Level" before the learner has earned a level
 */
const YOUR_LEVEL_THRESHOLDS = {
    Beginner: CHALLENGE_V1_TARGETS.Beginner,
    Intermediate: 20,
    Expert: 10
};

function showActiveLevelCard() {
    if (!activeLevelCard) {
        return;
    }

    activeLevelCard.hidden = false;
    activeLevelCard.removeAttribute("aria-hidden");
}

function hideActiveLevelCard() {
    if (!activeLevelCard) {
        return;
    }

    activeLevelCard.hidden = true;
    activeLevelCard.setAttribute(
        "aria-hidden",
        "true"
    );
...
function updateActiveLevelStatus(
    ...
    /*
     * IMPORTANT:
     * Nothing is displayed until the learner reaches one of the
     * defined progression thresholds.
     *
     * Highest earned threshold wins:
     * Expert (10+) > Intermediate (20+) > Beginner (50/50).
     */
    if (
        expertCompleted <
        YOUR_LEVEL_THRESHOLDS.Expert
        &&
        intermediateCompleted <
        YOUR_LEVEL_THRESHOLDS.Intermediate
        &&
        beginnerCompleted <
        YOUR_LEVEL_THRESHOLDS.Beginner
    ) {
        hideActiveLevelCard();
        return;
    }
    ...
    /*
     * The card is revealed only now, after a genuine milestone
     * has been reached and the correct status has been calculated.
     */
    showActiveLevelCard();
