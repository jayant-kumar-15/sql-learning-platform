...
/*
 * The level card itself is hidden until the user reaches the first
 * real milestone. This prevents a locked/empty level badge from
 * appearing before the user has earned anything.
 */
const activeLevelCard =
    document.getElementById("active-level-card");
...
function updateActiveLevelStatus(
    ...
    /*
     * ============================================================
     * ACTIVE LEVEL UNLOCK RULES
     * ------------------------------------------------------------
     * No level card is shown until the user earns a real milestone.
     *
     * Priority:
     *   1. Expert  >= 10 completed -> Expert / 💀 BOSS
     *   2. Intermediate >= 20      -> Intermediate / ⭐
     *   3. Beginner >= 50          -> Easy / 50/50 ✔️
     *
     * Once a higher level is unlocked it replaces the lower level
     * in the "Your Level" card.
     * ============================================================
     */
     ...
     *
     * This is important on first load and after a reset so users do
     * NOT see "Expert - 🔒", "Easy - 0/50", etc.
     */
    if (activeLevelCard) {
        activeLevelCard.style.display = "none";
    }
    ...
    if (
        expertCompleted >=
        INTERMEDIATE_STAR_MILESTONES.length &&
        expertCompleted >= 10
    ) {
    ...
    /*
     * Intermediate becomes the active level after 20 completed.
     */
    else if (
        intermediateCompleted >= 20
    ) {
    ...
    /*
     * Beginner becomes visible only after all 50 Beginner
    ...
    /*
     * No milestone yet: keep Your Level completely hidden.
     */
    if (!unlocked) {
        ...
        if (activeLevelCard) {
            activeLevelCard.style.display = "block";
        }
    }
...
console.log(
    "✅ challenge.js loaded successfully — Challenge Fix v28"
);
