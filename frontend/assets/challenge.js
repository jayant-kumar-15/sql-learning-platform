...
/*
 * INITIAL ACTIVE-LEVEL LOADING STATE
 * -----------------------------------
 * The HTML intentionally starts with "Loading…" instead of a guessed
 * "Easy - 0/50". The real value is calculated only after all challenge
 * JSON files and saved browser progress have been restored.
 */
function revealActiveLevelStatus() {
    if (!activeLevelStatus) {
        return;
    }

    activeLevelStatus.classList.remove(
        "active-level-status-loading"
    );

    activeLevelStatus.removeAttribute(
        "aria-busy"
    );
}
...
function updateActiveLevelStatus(
    ...
    /*
     * Reveal the calculated level only after the real progress state
     * has been applied. This removes the refresh-time "Easy" flash.
     */
    revealActiveLevelStatus();
