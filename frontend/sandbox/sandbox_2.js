 ...
 * ------------------------------------------------------------
 * Uses the existing Run Query button and existing click listener.
 * No query execution, result handling, or database logic is changed.
 * Desktop: button stays in the worksheet toolbar.
 * Mobile: button is moved below the editor into the dedicated action
 * area, matching the locked Playground/Challenges mobile design.
 * ============================================================ */
(function setupResponsiveRunQueryButton() {

    const mobileRunQueryActions =
        document.getElementById("mobile-run-query-actions");

    if (!runQueryButton || !mobileRunQueryActions) {
        return;
    }

    const originalParent =
        runQueryButton.parentElement;
    ...
    const originalNextSibling =
        runQueryButton.nextSibling;

    function syncRunQueryButtonPosition() {
        ...
        if (isMobile) {
            ...
            if (
                ...
                mobileRunQueryActions.appendChild(
                    runQueryButton
                );
            }
