...
function showDatabaseActionStatus(message, type) {
    /*
     * All Database Explorer mutations use this single path so the
     * user gets an immediate confirmation in the existing Sandbox
     * status/message area. No new UI or database logic is introduced.
     */
...
function showStatus(
    ...
    sandboxStatus.style.display = "block";
    sandboxStatus.style.visibility = "visible";
...
function displayTableData(databaseName, tableName) {
    ...
    ui.body.querySelector("#ui-commit-data").addEventListener("click", function () {
        try {
            ...
            ui.close();
            displayTableData(databaseName, tableName);
            showDatabaseActionStatus("✅ Table '" + tableName + "' updated successfully. Changes committed.", "success");
        } catch (error) {
            showStatus("❌ " + getSQLiteErrorMessage(error), "error");
        }
    });
