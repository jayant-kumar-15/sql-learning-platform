/*
 * SQL Learning Platform - Tutorial JavaScript V5
 * ============================================================
 * FILE PATH
 * ----------
...
(() => {
    ...
    /* ============================================================
     * ADVANCED CONCEPT LEARNING GUIDES
     * ------------------------------------------------------------
     * These guides intentionally sit between the definition and the
     * SQL example. They explain the "mental execution flow" of an
     * advanced query before a learner is asked to run it.
     *
     * REVISION-SENSITIVE:
     * - These examples are instructional and must remain compatible
     *   with the current Banking/Healthcare schema where SQL is shown.
     * - Keep the visual explanation independent from the SQL engine so
     *   a lesson can still be studied when execution is unavailable.
     * ============================================================ */

    const ADVANCED_LEARNING_GUIDES = {
        "Subqueries": {
            title: "🧩 How a Subquery Works",
            definition: "A subquery is a complete SQL query placed inside another query. The inner query produces a value or result set that the outer query uses to make its decision. A useful way to learn subqueries is to think of them as solving one smaller question first, then feeding that answer into the main question.",
            steps: [
                ["1. Ask the inner question", "Example: What is the average loan amount?"],
                ["2. Produce the benchmark", "The inner SELECT returns one value, such as AVG(principal_amount)."],
                ["3. Return to the outer query", "The outer SELECT compares each loan against that benchmark."],
                ["4. Keep matching rows", "Only loans satisfying the outer WHERE condition remain."]
            ],
            example: "SELECT loan_id, principal_amount\nFROM loans\nWHERE principal_amount > (\n    SELECT AVG(principal_amount)\n    FROM loans\n);",
            tip: "Before writing the full query, write and run the inner SELECT alone. If its result makes sense, place it inside the outer query."
        },

        "EXISTS and NOT EXISTS": {
            title: "🔎 EXISTS — Think 'Does a Match Exist?'",
            definition: "EXISTS does not need the matching rows themselves. It asks whether the subquery can find at least one related row for the current outer row. NOT EXISTS reverses that test. This makes EXISTS especially useful for questions such as 'customers who have at least one loan' or 'customers who have no loan'.",
            steps: [
                ["1. Pick one outer row", "Take the current customer from customers."],
                ["2. Check the related table", "Look for a loan whose customer_id matches."],
                ["3. Stop at the first match", "EXISTS is satisfied as soon as one matching row exists."],
                ["4. Keep or reject the outer row", "EXISTS keeps it; NOT EXISTS keeps it only when no match exists."]
            ],
            example: "SELECT c.customer_id, c.first_name, c.last_name\nFROM customers c\nWHERE EXISTS (\n    SELECT 1\n    FROM loans l\n    WHERE l.customer_id = c.customer_id\n);",
            tip: "SELECT 1 inside EXISTS is conventional because EXISTS cares about whether a row exists, not which columns the subquery returns."
        },

        "CTEs — WITH": {
            title: "🧱 CTE — Break One Big Problem into Stages",
            definition: "A Common Table Expression (CTE) is a named temporary result defined with WITH for one SQL statement. It is not a permanent table. Its biggest learning advantage is that a complicated business problem can be divided into small, testable stages.",
            steps: [
                ["Stage 1 — Prepare", "Build the clean base dataset you actually need."],
                ["Stage 2 — Calculate", "Aggregate, join or enrich the prepared rows."],
                ["Stage 3 — Analyze", "Rank, classify or compare the calculated result."],
                ["Stage 4 — Present", "Select only the final columns the user needs."]
            ],
            example: "WITH customer_totals AS (\n    SELECT a.customer_id,\n           SUM(t.amount) AS total_amount\n    FROM accounts a\n    JOIN transactions t ON t.account_id = a.account_id\n    GROUP BY a.customer_id\n)\nSELECT customer_id, total_amount\nFROM customer_totals\nWHERE total_amount > 100000;",
            tip: "When debugging a CTE, temporarily run each CTE's SELECT separately. This makes it much easier to find which stage produced an unexpected result."
        },

        "Correlated Subquery": {
            title: "🔁 Correlated Subquery — The Inner Query Depends on the Outer Row",
            definition: "A correlated subquery is different from an independent subquery because the inner query refers to a column from the current outer row. Conceptually, the database asks the inner question in the context of each outer row. This is powerful for per-group comparisons, but it can be harder to reason about and may have clearer alternatives such as window functions.",
            steps: [
                ["1. Pick an outer row", "Example: one particular loan."],
                ["2. Pass its group key inward", "The current customer_id becomes available to the inner query."],
                ["3. Calculate that customer's benchmark", "The inner query finds the customer's average loan amount."],
                ["4. Compare", "The outer loan is kept when its amount is above that benchmark."]
            ],
            example: "SELECT l.loan_id, l.customer_id, l.principal_amount\nFROM loans l\nWHERE l.principal_amount > (\n    SELECT AVG(l2.principal_amount)\n    FROM loans l2\n    WHERE l2.customer_id = l.customer_id\n);",
            tip: "Look for the outer alias inside the subquery. Here, l.customer_id is what makes the subquery correlated."
        },

        "ROW_NUMBER, RANK and DENSE_RANK": {
            title: "🏆 Ranking Functions — Why Do the Numbers Behave Differently?",
            definition: "ROW_NUMBER, RANK and DENSE_RANK are window functions used to assign a position to rows after ordering them. They do not collapse rows like GROUP BY. The key difference appears when two or more rows have the same ordering value.",
            steps: [
                ["ROW_NUMBER", "Every row gets a unique position: 1, 2, 3, 4."],
                ["RANK", "Ties share a rank and the next rank has a gap."],
                ["DENSE_RANK", "Ties share a rank but the next rank does not have a gap."],
                ["PARTITION BY", "If used, ranking restarts separately for each group."]
            ],
            example: "SELECT loan_id, principal_amount,\n       ROW_NUMBER() OVER (ORDER BY principal_amount DESC) AS row_num,\n       RANK() OVER (ORDER BY principal_amount DESC) AS rank_num,\n       DENSE_RANK() OVER (ORDER BY principal_amount DESC) AS dense_rank_num\nFROM loans;",
            tip: "For amounts 900, 800, 800, 700: RANK gives 1, 2, 2, 4; DENSE_RANK gives 1, 2, 2, 3. This is the classic 'gap' difference."
        },

        "LAG and LEAD": {
            title: "↔️ LAG & LEAD — Look at the Previous or Next Row",
            definition: "LAG reads a value from an earlier row in the defined order, while LEAD reads a value from a later row. They let you compare a row with its neighbor without writing a self-join. The ORDER BY inside OVER is therefore critical because it defines what 'previous' and 'next' actually mean.",
            steps: [
                ["1. Define the group", "PARTITION BY account_id keeps comparisons inside each account."],
                ["2. Define the timeline", "ORDER BY transaction_date, transaction_id establishes row order."],
                ["3. Look backward", "LAG(amount) returns the previous transaction amount."],
                ["4. Look forward", "LEAD(amount) returns the next transaction amount."]
            ],
            example: "SELECT account_id,\n       transaction_id,\n       transaction_date,\n       amount,\n       LAG(amount)  OVER (PARTITION BY account_id ORDER BY transaction_date, transaction_id) AS previous_amount,\n       LEAD(amount) OVER (PARTITION BY account_id ORDER BY transaction_date, transaction_id) AS next_amount\nFROM transactions;",
            tip: "The first row in a group has no previous row, so LAG normally returns NULL there. Likewise, the last row has no next row, so LEAD returns NULL."
        },

        "Running Totals": {
            title: "📈 Running Total — Keep Every Row, Accumulate the Value",
            definition: "A running total is a cumulative calculation. Unlike GROUP BY, which produces one row per group, a window SUM keeps every original transaction row and adds the amount accumulated up to the current row.",
            steps: [
                ["Row 1", "Running total = amount on row 1."],
                ["Row 2", "Running total = row 1 + row 2."],
                ["Row 3", "Running total = row 1 + row 2 + row 3."],
                ["Continue", "Each new row extends the cumulative total."]
            ],
            example: "SELECT account_id, transaction_id, transaction_date, amount,\n       SUM(amount) OVER (\n           PARTITION BY account_id\n           ORDER BY transaction_date, transaction_id\n           ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW\n       ) AS running_total\nFROM transactions;",
            tip: "The window ORDER BY defines the accumulation sequence. If the order is wrong, the running total can be logically wrong even though the SQL is syntactically valid."
        },

        "Top-N Per Group": {
            title: "🥇 Top-N Per Group — Rank First, Filter Second",
            definition: "Top-N per group means finding the top N rows inside every business group, such as the top three loans for every customer. A common mistake is using ORDER BY ... LIMIT because LIMIT normally gives one global top-N result rather than restarting the ranking for every group.",
            steps: [
                ["1. Define the group", "Example: customer_id."],
                ["2. Rank inside each group", "Use ROW_NUMBER, RANK or DENSE_RANK with PARTITION BY."],
                ["3. Keep the desired ranks", "Filter the ranked result in an outer query or CTE."],
                ["4. Preserve ties when required", "Choose RANK/DENSE_RANK instead of ROW_NUMBER when the business rule requires ties."]
            ],
            example: "WITH ranked_loans AS (\n    SELECT customer_id, loan_id, principal_amount,\n           ROW_NUMBER() OVER (\n               PARTITION BY customer_id\n               ORDER BY principal_amount DESC\n           ) AS rn\n    FROM loans\n)\nSELECT customer_id, loan_id, principal_amount\nFROM ranked_loans\nWHERE rn <= 3;",
            tip: "Interview clue: whenever you hear 'top N for each department/customer/account', think PARTITION BY + ranking + outer filtering."
        },

        "Deduplication with ROW_NUMBER": {
            title: "🧹 Deduplication — Define the Duplicate and the Survivor",
            definition: "SQL deduplication is not simply 'remove duplicates'. A production-quality solution must define which columns make rows duplicates and which record should survive. ROW_NUMBER lets you mark the preferred row as 1 and the other versions as 2, 3, and so on.",
            steps: [
                ["1. Define duplicate identity", "For example, rows sharing the same email."],
                ["2. Define the survivor rule", "Keep the most recently registered record."],
                ["3. Assign row numbers", "PARTITION BY email and ORDER BY registration_date DESC."],
                ["4. Keep rn = 1", "The preferred record survives; other versions are excluded."]
            ],
            example: "WITH ranked AS (\n    SELECT customer_id, email, registration_date,\n           ROW_NUMBER() OVER (\n               PARTITION BY email\n               ORDER BY registration_date DESC, customer_id DESC\n           ) AS rn\n    FROM customers\n    WHERE email IS NOT NULL\n)\nSELECT customer_id, email, registration_date\nFROM ranked\nWHERE rn = 1;",
            tip: "Always make the survivor ordering deterministic. Adding customer_id as a tie-breaker prevents two equally dated rows from producing an unpredictable winner."
        },

        "Multi-Stage CTE Analysis": {
            title: "🧠 Advanced Query Flow — Base → Aggregate → Rank → Filter",
            definition: "Many real-world SQL tasks are multi-stage problems. Instead of trying to write one giant SELECT, identify the grain and business purpose of each stage. CTEs make those stages visible and testable.",
            steps: [
                ["Base", "Join the source tables and establish the correct business grain."],
                ["Aggregate", "Calculate one metric per customer, account or other entity."],
                ["Rank", "Use a window function on the aggregated result."],
                ["Filter", "Select the required rank or business threshold."]
            ],
            example: "WITH customer_totals AS (\n    SELECT a.customer_id, SUM(t.amount) AS total_amount\n    FROM accounts a\n    JOIN transactions t ON t.account_id = a.account_id\n    GROUP BY a.customer_id\n), ranked AS (\n    SELECT customer_id, total_amount,\n           DENSE_RANK() OVER (ORDER BY total_amount DESC) AS rnk\n    FROM customer_totals\n)\nSELECT customer_id, total_amount\nFROM ranked\nWHERE rnk <= 3;",
            tip: "If a query feels impossible to write in one step, stop and name the intermediate business result you need. That usually becomes your first CTE."
        },

        "Interview: Window Function vs GROUP BY": {
            title: "⚖️ Window Function vs GROUP BY — The Grain Test",
            definition: "The fastest way to choose between GROUP BY and a window function is to ask: 'Do I want to collapse rows, or keep the detail rows?' GROUP BY reduces the result to one row per grouping key. A window function calculates across related rows while preserving the original row grain.",
            steps: [
                ["GROUP BY", "100 transactions for an account can become 1 account-level summary row."],
                ["Window function", "The same 100 transactions remain 100 rows, with an additional account-level metric."],
                ["Decision question", "Do I need detail + summary together? If yes, consider a window function."],
                ["Validation", "Check the expected number of rows before and after the calculation."]
            ],
            example: "SELECT account_id,\n       transaction_id,\n       amount,\n       SUM(amount) OVER (PARTITION BY account_id) AS account_total\nFROM transactions;",
            tip: "This 'row grain' test is one of the most useful habits for SQL interviews and real production work."
        }
    };

    function renderAdvancedLearningGuide(lesson) {
        const guide = ADVANCED_LEARNING_GUIDES[lesson.title];
        if (!guide) return "";

        return `
            <section class="advanced-learning-guide">
                <div class="advanced-guide-heading">
                    <span class="advanced-guide-icon">🧭</span>
                    <div>
                        <h3>${escapeHtml(guide.title)}</h3>
                        <p>Understand the logic before memorizing the syntax.</p>
                    </div>
                </div>

                <div class="advanced-guide-definition">
                ...
                <div class="advanced-flow">
                    ${guide.steps.map((step, index) => `
                        <div class="advanced-flow-step">
                            <span class="advanced-flow-number">${index + 1}</span>
                            <div>
                                <strong>${escapeHtml(step[0])}</strong>
                        ...
                        ${index < guide.steps.length - 1
                            ? `<div class="advanced-flow-arrow">↓</div>`
                            : ""}
                    `).join("")}
                </div>

                <div class="advanced-guide-example">
                    <div class="advanced-example-title">💡 Example to visualize the idea</div>
                    <pre><code>${escapeHtml(guide.example)}</code></pre>
                </div>

                ${lesson.title === "ROW_NUMBER, RANK and DENSE_RANK"
                    ? `
                        <div class="rank-visual">
                            <div class="rank-visual-title">🏆 Same values, different ranking behavior</div>
                            <div class="rank-table-wrap">
                                <table class="rank-table">
                                    ...
                                    <tbody>
                                        ...
                                        <tr><td>800</td><td>2</td><td>2</td><td>2</td></tr>
                                        <tr><td>800</td><td>3</td><td>2</td><td>2</td></tr>
                                        <tr><td>700</td><td>4</td><td><strong>4</strong></td><td><strong>3</strong></td></tr>
                                    </tbody>
                                </table>
                            </div>
                            <p class="rank-note">
                                <strong>Key idea:</strong> RANK leaves a gap after a tie (1, 2, 2, 4).
                                DENSE_RANK does not (1, 2, 2, 3). ROW_NUMBER never shares a number.
                            </p>
                        </div>
                    `
                    : ""}

                <div class="advanced-guide-tip">
                    <strong>🎯 Learning tip</strong>
                    <span>${escapeHtml(guide.tip)}</span>
                </div>
            </section>
        `;
    }
    ...
    function renderLesson() {
        ...
        container.innerHTML = `
            ...
            <article class="tutorial-lesson">
                ...
                ${renderAdvancedLearningGuide(lesson)}
