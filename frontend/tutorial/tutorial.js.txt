...
(() => {
     ...
     * ---------------------
     * Keep every learning track collapsed on first load.
     * The learner can expand any section using its arrow/button.
     *
     * IMPORTANT:
     * - Lesson rendering still uses the existing activeTrack.
     * - Clicking a track/lesson continues to use the existing
     *   setActiveTrack() and toggleTrack() logic.
     * - Homepage deep-links are still allowed to open their
     *   requested track during initialization.
     */
    const collapsedTracks = new Set(
        Object.keys(TUTORIAL_CONTENT)
    );
    ...
    function buildSidebar() {
        const sidebarList = $(".tutorial-roadmap");
     ...
     * MOBILE NAVIGATION
     * ============================================================
     * Desktop keeps the existing Tutorial navigation unchanged.
     * On normal mobile view, the hamburger below the brand toggles
     * the existing navigation without touching lesson/sidebar logic.
     */
    function initializeMobileNavigation() {
        const button = $("#tutorial-mobile-nav-button");
        const header = $(".tutorial-header");
    ...
    function initialize() {
        /*
         * ============================================================
         * HOMEPAGE TRACK DEEP-LINK SUPPORT
         * ============================================================
         * The homepage learning-path cards can open Tutorial with a
         * `track` query parameter (beginner/intermediate/expert/interview).
         * This reads that parameter once during initialization so the
         * requested tutorial section opens immediately.
         *
         * IMPORTANT: This only changes the initial tutorial selection.
         * Existing sidebar navigation, saved state, lesson content and
         * browser-SQL execution remain unchanged.
         */
        const requestedTrack = new URLSearchParams(window.location.search).get('track');

        if (requestedTrack && TUTORIAL_CONTENT[requestedTrack]) {
            activeTrack = requestedTrack;
            activeLessonIndex = 0;
            collapsedTracks.clear();
            Object.keys(TUTORIAL_CONTENT).forEach((key) => {
                if (key !== requestedTrack) {
                    collapsedTracks.add(key);
                }
            });
        }
