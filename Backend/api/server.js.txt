...
const analyticsModule = require("../routes/analyticsRoutes");
const analyticsRoutes = analyticsModule.router;
const ensureRealtimeTable = analyticsModule.ensureRealtimeTable;
...
initializeDatabase(function (error) {
     ...
     */
    ensureRealtimeTable(function (realtimeError) {
        if (realtimeError) {
            console.error("❌ Realtime analytics initialization failed:", realtimeError.message);
            process.exit(1);
        }
