...
router.post(
    ...
    function (req, res) {
        ...
        /* True only for a real page load; false is a live heartbeat. */
        ...
        function updatePresence() {
            db.run(
                ...
                function (presenceError) {
                    if (presenceError) {
                        console.error("Realtime traffic tracking failed:", presenceError.message);
                    }
        ...
        /* Heartbeats update presence only; page loads also create history. */
        if (!trackHistory) {
...
router.get(
    ...
    function (req, res) {
        ...
        /* Ensure the realtime presence table exists before the admin read.
         * This keeps the route backward-compatible with the existing server.js
         * and avoids a blank/500 Live Visitors value after a fresh database.
         */
        ensureRealtimeTable(function (tableError) {
            if (tableError) {
                console.error("Realtime table setup failed:", tableError.message);
                return res.status(500).json({
                    success: false,
                    message: "Unable to initialize live traffic analytics."
                });
            }
