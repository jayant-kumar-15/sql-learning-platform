...
router.get(
    ...
    function (req, res) {
        ...
        if (range === "12w") {
             ...
             * WEEKLY MODE
             * ------------
             * Group by the actual ISO week start date instead of returning
             * one row per calendar day. This guarantees that the Weekly
             * chart receives one point per week.
             *
             * PostgreSQL date_trunc('week', ...) starts the bucket on
             * Monday. The displayed label remains an ISO week such as
             * 2026-W38.
             */
            startExpression = "CURRENT_DATE - INTERVAL '83 days'";
            groupExpression = "date_trunc('week', visit_date)";
            labelExpression = "to_char(date_trunc('week', visit_date), 'IYYY-\"W\"IW')";
            orderExpression = "MIN(date_trunc('week', visit_date))";
        } else if (range === "12m") {
