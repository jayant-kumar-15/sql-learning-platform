 ...
 * Persistent application data is stored in PostgreSQL; the existing
 * learning SQLite/browser-WASM paths are intentionally unaffected.
 *
 * ENDPOINTS
 * ---------
 ...
 * persistent PostgreSQL. The browser receives the raw session token only through
 * an HttpOnly cookie.
 *
...
router.post(
    ...
    function (req, res) {
        ...
        /*
         * The private admin login form calls this value `identifier`.
         * Email is still the canonical account lookup key. Supporting
         * both names keeps the API compatible with older clients.
         */
        const email =
            String(
                req.body.identifier ||
